...
Func Check_source_integrity($linux_live_file)
	...
	If ReadSetting("Install_Parameters","automatic_recognition")<>"yes" Then
		...
		if $release_number <> -1 Then
			Step2_Check("good")
			Sleep(100)
			GUI_Show_Check_status(Translate("Verifying") & " OK"&@CRLF& Translate("This version is compatible and its integrity was checked")&@CRLF&Translate("Recognized ISO")&" : "&@CRLF& @CRLF & @TAB &ReleaseGetDescription($release_number))
	...
	If $temp_index > 0 Then
		If ReleaseGetMD5($temp_index) = "ANY" Then
			...
			GUI_Show_Check_status(Translate("This version is compatible and its integrity was checked")&@CRLF&Translate("Recognized ISO")&" : "&@CRLF& @CRLF & @TAB &ReleaseGetDescription($temp_index))
			$release_number = $temp_index
			Check_If_Default_Should_Be_Used($release_number)
	...
	If $temp_index > 0 Then
		...
		GUI_Show_Check_status(Translate("Verifying") & " OK"&@CRLF& Translate("This version is compatible and its integrity was checked")&@CRLF&Translate("Recognized ISO")&" : "&@CRLF& @CRLF & @TAB &ReleaseGetDescription($temp_index))
		Step2_Check("good")
		$release_number = $temp_index
	...
	Else
		...
		If $temp_index > 0 Then
			...
			$release_number = $temp_index
			GUI_Show_Check_status(Translate("You have the right ISO file but it is corrupted or was altered") &". "&Translate("Please download it again")&"."&@CRLF&Translate("However, GenOS USB Creator will try to use same install parameters as for") & @CRLF & @TAB & @TAB& ReleaseGetDescription($release_number))
			Step2_Check("warning")
		...
		Else
			...
			GUI_Show_Check_status(Translate("This Linux is not in the compatibility list")& "." & @CRLF &Translate("However, GenOS USB Creator will try to use same install parameters as for") & @CRLF & @CRLF & @TAB & ReleaseGetDescription($release_number))
